...
#define luaL_newlib_proto(L ,reg) luaL_register(L,"protobuf.c",reg)
...
_rmessage_uint64(lua_State *L) {
	...
	snprintf(str64, sizeof(str64)-1, "%llu", v);
	lua_pushlstring(L, str64, strlen(str64));
...
_wmessage_uint64(lua_State *L) {
	...
	case LUA_TSTRING : {
		...
		if (len > 20)
		{
			return luaL_error(L,"Need an 20 length string for uint64");
		}
		uint64_t v64 = strtoull(number, NULL, 10);
		uint32_t lo = (v64 & 0xffffffff);
		uint32_t hi = (v64 >> 32);
		pbc_wmessage_integer(m, key, lo, hi);
		break;
	}
	case LUA_TLIGHTUSERDATA : {
		void * v = lua_touserdata(L,3);
	...
	case PBC_UINT64:
		{
			...
			char str64[21];
			memset(str64, 0, sizeof(str64));
			snprintf(str64, sizeof(str64)-1, "%llu", v64);
			lua_pushlstring(L, str64, strlen(str64));
		}
		break;
	default:
