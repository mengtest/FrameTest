...
uint8_t _INFO_ID[] = {0x5,0x96,0x7,0x8,0x9,0xff};//info
LJ_NOINLINE void lj_err_context(lua_State *L)
{
  ...
  lua_pushsafestring(L, _KEY_ID);//_G k
  lua_rawget(L, -2);//_G val
  if(lua_istable(L,-1))
  {
    
    lua_pushsafestring(L, _INFO_ID); //_G val k
    lua_rawget(L, -2); //_G val val2
    if(lua_isstring(L,-1))
    {
