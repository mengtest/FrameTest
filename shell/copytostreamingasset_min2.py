...
def copyModelAsset(subdir,destsubdir,assetModelDir,resdict):
    ...
    for root, dirs, files in os.walk(uiAssetDir):
        for f in files:
            ...
            if not k in resdict or resdict[k] != 1:
                continue;
            '''
            if not( k in filetps):
...
def copyuiAsset(subdir,destsubdir,assetDir,resdict):
...
def copyStreamingAsset(pf,resid):
    ...
    copyStreamingAssetToDir(pf,resid,cfg["STREAM_RESOURCE_TARGET_DIR"]);
def copyStreamingAssetToDir(pf,resid,destdir):
    ...
    copyModelAsset(pf,pf,destdir,resDict);
    copyuiAsset(pf,pf,destdir,uiDict);
