...
from copytostreamingasset_min import copyStreamingAsset;
from encdll import encodeDll2;
import io;
...
def checkAppSvn(d,channel,pf,resid):
    
    print "begin to update:",d;
    client.update(d);
    
    cfg = readIni(basedir+"/"+pf+"_"+channel+"_apprev.log");
    ...
    '''
def checkResSvn(d,channel,pf,resid):
    
    showLog("begin to update:",d);
    client.update(d);
    
    cfg = readIni(basedir+"/"+pf+"_"+channel+"_resrev.log");
    startv = int(cfg.get("rev") or 0);
    if startv<=0:
        return True,True;
    info = client.info(d);
...
def writeAppSvnInfo(d,channel):
    cfg = readIni(basedir+"/"+pf+"_"+channel+"_apprev.log");
    
    info = client.info(d+"/main");
    cfg["prev"] = cfg.get("rev") or 0;
    cfg["rev"]=info.revision.number;
    writeIni(cfg,basedir+"/"+pf+"_"+channel+"_apprev.log");

def writeResSvnInfo(d,channel):
    cfg = readIni(basedir+"/"+pf+"_"+channel+"_resrev.log");
    
    info = client.info(d);
    cfg["prev"] = cfg.get("rev") or 0;
    cfg["rev"]=info.revision.number;
    writeIni(cfg,basedir+"/"+pf+"_"+channel+"_resrev.log");
...
def deployRes(cfg,d,resid,channel,force):
    showLog("begin deploy res:");
     
    client.update(d);
    showLog("update res svn end,begin check change ");
    res_change,filterchange = checkResSvn(d,channel,pf,resid);
...
def deployPkg(cfg,pf,channel,resid):
    ...
    "iOS":"ProjectBuild.BuildForIPhone",
    "iOSde":"ProjectBuild.BuildForIPhoneLocal"}[pf];
...
def doAndroidRepackage(cfg,pkgpath,fn):
    ...
    encodeDll2("android",dllpath,dllpath,"android/release/app_temp/Assembly-CSharp.dll");    
    ...
    if not "UPLOAD" in cfg or  cfg["UPLOAD"] == "true":
        uploadDll();
def uploadDll():
    ...
    with SCPClient(ssh.get_transport()) as scp:
        print scp.put("android/release/app_temp/Assembly-CSharp.dll",cfg["UPLOAD_RES_PATH"]+"/android/release/app_temp/Assembly-CSharp.dll");
        print scp.put("android/release/app_temp/app.info",cfg["UPLOAD_RES_PATH"]+"/android/release/app_temp/app.info");
    ssh.close();    
...
def deployApp(cfg,d,pf,channel,subchannel,resid,forceapp):
    reschange,appchange,luachange,filterchange = checkAppSvn(d,channel,pf,resid);
...
def doHotupdate(cfg,pf,channel,subchannel,resid,force):
    reschange,filterchange =  deployRes(cfg,cfg["RES_PATH"],resid,channel,force);
    ...
    copyRes(pf,pf,reschange,uichange,luachange,force);        
    showLog("end deploy res,begin jenkins job");
        
    sys.stdout.flush();
    writeAppSvnInfo(cfg["APP_PATH"],channel);
    writeResSvnInfo(cfg["RES_PATH"],channel);
...
def doDeployRes(cfg,pf,channel,subchannel,resid,force):
    reschange =  deployRes(cfg,cfg["RES_PATH"],resid,channel,force);
    waitAllCmd();
    writeResSvnInfo(cfg["RES_PATH"],channel); 
def doDeployApp(cfg,pf,channel,subchannel,resid,force):
    ...
    if   uichange or luachange or force:
        showLog("begin copy res");
        copyRes(pf,pf,False,uichange,luachange,force);        
...
def doDeploy(cfg,pf,channel,subchannel,target,resid,force):
    ...
    if target == "all":
        ...
        copyRes(pf,pf,True,False,False,force);
    elif target == "app":
        doDeployApp(cfg,pf,channel,subchannel,resid,force);
        copyRes(pf,pf,False,True,True,force);
    elif target == "pkg":
        deployPkg(cfg,pf,channel,resid);
    ...
    elif target == "hotupdate":
        doHotupdate(cfg,pf,channel,subchannel,resid,force);
