...
INTERPOLATED_REGULAR_STRING_START:   '$"'
    { self.interpolatedStringLevel = self.interpolatedStringLevel+1; self.interpolatedVerbatiums.append(False); verbatium = False; } -> pushMode(INTERPOLATION_STRING);
INTERPOLATED_VERBATIUM_STRING_START: '$@"'
    { self.interpolatedStringLevel = self.interpolatedStringLevel+1; self.interpolatedVerbatiums.append(True); verbatium = True; }  -> pushMode(INTERPOLATION_STRING);
...
DOUBLE_QUOTE_INSIDE:           '"' { self.interpolatedStringLevel = self.interpolatedStringLevel-1; self.interpolatedVerbatiums.pop();
 verbatium = (self.interpolatedVerbatiums.size() > 0 and  self.interpolatedVerbatiums.peek() or False); } -> popMode;
...
CLOSE_BRACE_INSIDE:             '}' { self.curlyLevels.pop(); }   -> skip, popMode;
FORMAT_STRING:                  ~'}'+;
