...
argument
	: (identifier COLON)? refout=(REF | OUT)? (VAR | ctype)? expression
	;
...
assignment_operator
	: ASSIGNMENT | OP_ADD_ASSIGNMENT | OP_SUB_ASSIGNMENT | OP_MULT_ASSIGNMENT | OP_DIV_ASSIGNMENT | OP_MOD_ASSIGNMENT | OP_AND_ASSIGNMENT | OP_OR_ASSIGNMENT | OP_XOR_ASSIGNMENT | OP_LEFT_SHIFT | right_shift_assignment
	;
...
relational_expression
	: shift_expression ((LT | GT | OP_LE | OP_GE) shift_expression | IS isType | AS ctype)*
	;
...
unary_expression
	...
	| OPEN_PARENS ctype CLOSE_PARENS unary_expression
...
primary_expression  // Null-conditional operators C# 6: https://msdn.microsoft.com/en-us/library/dn986595.aspx
	: pe=primary_expression_start bracket_expression*
	  ((member_access | method_invocation | OP_INC | OP_DEC | OP_PTR identifier) bracket_expression*)*
	;
...
primary_expression_start
	...
	| NEW (ctype (object_creation_expression
	...
	| TYPEOF OPEN_PARENS (unbound_type_name | ctype | VOID) CLOSE_PARENS   #typeofExpression
	...
	| DEFAULT OPEN_PARENS ctype CLOSE_PARENS                         #defaultValueExpression
	| ASYNC? DELEGATE (OPEN_PARENS explicit_anonymous_function_parameter_list? CLOSE_PARENS)? block #anonymousMethodExpression
	| SIZEOF OPEN_PARENS ctype CLOSE_PARENS                          #sizeofExpression
...
explicit_anonymous_function_parameter
	: refout=(REF | OUT)? ctype identifier
	;
...
from_clause
	: FROM ctype? identifier IN expression
	;
...
combined_join_clause
	: JOIN ctype? identifier IN expression ON expression EQUALS expression (INTO identifier)?
	;
...
ordering
	: expression cdir=(ASCENDING | DESCENDING)?
	;
...
local_variable_declaration
	: local_variable_type local_variable_declarator ( COMMA  local_variable_declarator)*
	;
...
local_constant_declaration
	: CONST ctype constant_declarators
	;
...
common_member_declaration
	...
	| event_declaration
	| conversion_operator_declarator (body | right_arrow expression SEMICOLON) // C# 6
...
conversion_operator_declarator
	: (IMPLICIT | EXPLICIT) OPERATOR ctype OPEN_PARENS arg_declaration CLOSE_PARENS
	;
...
struct_member_declaration
	: attributes? all_member_modifiers?
	  (common_member_declaration | FIXED ctype fixed_size_buffer_declarator+ SEMICOLON)
	;
...
interface_member_declaration
	: attributes? NEW?
	  ...
	  | EVENT ctype identifier SEMICOLON)
	;
...
local_variable_initializer_unsafe
	: STACKALLOC ctype OPEN_BRACKET expression CLOSE_BRACKET
	;
...
event_declaration
	: EVENT ctype (variable_declarators SEMICOLON | member_name OPEN_BRACE event_accessor_declarations CLOSE_BRACE)
	;
...
property_declaration // Property initializer & lambda in properties C# 6
	: member_name (OPEN_BRACE accessor_declarations CLOSE_BRACE (ASSIGNMENT variable_initializer SEMICOLON)? | right_arrow expression SEMICOLON)
	;

constant_declaration
	: CONST ctype constant_declarators SEMICOLON
	;

indexer_declaration // lamdas from C# 6
	: THIS OPEN_BRACKET formal_parameter_list CLOSE_BRACKET (OPEN_BRACE accessor_declarations CLOSE_BRACE | right_arrow expression SEMICOLON)
	;
...
method_declaration // lamdas from C# 6
	: method_member_name type_parameter_list? OPEN_PARENS formal_parameter_list? CLOSE_PARENS
	    type_parameter_constraints_clauses? (method_body | right_arrow expression SEMICOLON)
	;
...
operator_declaration // lamdas form C# 6
	: OPERATOR overloadable_operator OPEN_PARENS arg_declaration
	       (COMMA arg_declaration)? CLOSE_PARENS (body | right_arrow expression SEMICOLON)
	;

arg_declaration
	: ctype identifier (ASSIGNMENT expression)?
	;
